	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header xCNModalHead">
				<label class="xCNTextModalHeard"><?php echo language('document/purreqcn', 'tPURTitleModalRoundBranch')?></label>
			</div>
			<div class="modal-body">
				<table id="otbTableRoundBranch" class="table table-striped xCNTableHead ">
					<thead>
						<tr>
							<th style="width:20%; text-align: center;"><?=language('document/purreqcn', 'tPURModalRoundBranchChoose')?> </th>
							<th style="width:80%; text-align: left;"><?=language('document/purreqcn', 'tPURModalRoundBranchReturnPDT')?></th>
						</tr>
					</thead>
					<tbody>
						<tr class="otrRoundBranch" style="cursor:pointer;" data-key="PUR1">
							...
							<td><?=language('document/purreqcn', 'tPURModalRoundBranchReturnROUND')?></td>
						</tr>
						...
						<tr class="otrRoundBranch" style="cursor:pointer;" data-key="PUR2">
							...
							<td><?=language('document/purreqcn', 'tPURModalRoundBranchReturnBCH')?></td>
						</tr>
					</tbody>
				</table>
			</div>
...
<script>
	...
	$('title').html("Purchase - Request Credit Note");
	...
	function JSvPURSelectRoundBranch(){
		...
		if(tRoundBranch == 'PUR1'){
			//ตามรอบ
			JSvMainpagePurreq();
		}else if(tRoundBranch == 'PUR2'){
			//ตามสาขา
			//Show Supplier
			setTimeout(function(){
				$('#odvModalSupplier').modal('show');
				//Call Type Sup
				var tRountGetTypeSup = '<?=$tROUTE_omnPurReqCNNew_gettypesupplier?>';
				$.ajax({
	...
	function JSxGetSupplier(elem,pnTypeSupCode,nPage,tType){
		...
		//Call Sup
		var tRountGetSup 	= '<?=$tROUTE_omnPurReqCNNew_getsupplier?>';
	...
	function JSvMainpagePurreq(tRoundBranch,pnTypeSupCode,pnSupCode){
		...
		var tRouteContent = '<?=$tROUTE_omnPurReqCNNew_mainpage?>';
		$.ajax({
			url     : tRouteContent,
			data 	: { 
				'pnSupCode' 	: pnSupCode ,
