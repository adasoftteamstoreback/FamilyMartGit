            ...
            <tbody>
                <?php if($aDataList['rtCode'] == 1){ ?>
                    <?php foreach($aDataList['raItems'] AS $key=>$aValue){  
                        ...
                        //packdata
                        $aPackdata = array(
                            'tRouteDelete'      => $tROUTE_omnPurReqCNNew_delete, 
                            'FTXrhDocNo'        => $aValue['FTXrhDocNo'],
                            'FNXrdSeqNo'        => $aValue['FNXrdSeqNo'],
                    ...
                    ?>
                        <tr data-docno="<?=$aValue['FTXrhDocNo'];?>" data-oldqty="<?=$aValue['FCXrdQty']?>" data-pdt="<?=$aValue['FTPdtCode']?>" data-seq="<?=$aValue['FNXrdSeqNo'];?>" class="xCNTableTrClickActive">    
                            <td nowrap class="text-center"><?=$aValue['FNXrdSeqNo']?></td>
                            <?php if($aValue['FTXrhDocType'] == 5){ ?>
                                <!-- <td nowrap class="text-left"><?=$aValue['FTPdtCode']?></td> -->
                            <?php }else{ ?>
                                <?php if($aValue['FTXrhStaPrcDoc']  == ''){ ?>
                                    <!-- <td nowrap class="text-right xWColorEditinLine">
                                        <div class="field a-field a-field_a1 page__field" style="padding: 0px; width: 100px;">
                                            <input id="oetFieldFTPdtCode<?=$aValue['FNXrdSeqNo']?>" name="oetFieldFTPdtCode<?=$aValue['FNXrdSeqNo']?>" class="xWPurreqPDTCode inputs field__input a-field__input" type="text" style="text-align: left;" data-valueold="<?=$aValue['FTPdtCode']?>" value="<?=$aValue['FTPdtCode']?>">
                                        </div>
                                    </td> -->
                                <?php }else{ ?>
                                    <!-- <td nowrap class="text-left"><?=$aValue['FTPdtCode']?></td> -->
                            ...
                            <td nowrap class="text-left"><?=$aValue['FTXrdBarCode']?></td>
                            <td nowrap class="text-left"><?=$aValue['FTXrdApOrAr']?></td>
                            <td nowrap class="text-left"><?=$aValue['FTPunName']?></td>

                            <?php if($aValue['FTXrhStaPrcDoc']  == ''){ ?>
                                <td nowrap class="text-right xWColorEditinLine">
                                    <div class="field a-field a-field_a1 page__field" style="padding: 0px;">
                                        <input id="oetFieldFCXrdQty<?=$aValue['FNXrdSeqNo']?>" name="oetFieldFCXrdQty<?=$aValue['FNXrdSeqNo']?>" maxlength='5' class="xCNInputNumberOnly inputsChange field__input a-field__input" type="text" style="text-align: right;" value="<?=$aValue['FCXrdQty']?>" autocomplete="off">
                                    </div>
                                </td>
                            <?php }else{ ?>
                                <td nowrap class="text-right"><?=$aValue['FCXrdQty']?></td>
                            <?php }; ?>

                            <td nowrap class="text-right"><?=number_format($aValue['FCXrdSalePrice'], 2)?></td>
                            <!-- <td nowrap class="text-right" id="oetFieldSetPrice<?=$aValue['FNXrdSeqNo']?>"><?=number_format($aValue['FCXrdSetPrice'], 2)?></td> -->
                            <!-- <td nowrap class="text-right" id="oetFieldFCXrdB4DisChg<?=$aValue['FNXrdSeqNo']?>"><?=number_format($aValue['FCXrdNet'], 2)?></td> -->
                            <?php if($aValue['FTXrhStaPrcDoc'] == ''){ 
                                $tStaBlockIcon = '';
                            }else{ 
            ...
            <?php if($ptRoundBranch == 'PUR2'){ ?>
                <?php 
                    if(empty($aDataList['raItems'])){
                        ...
                        if($aDataList['raItems'][0]['FTXrhStaPrcDoc'] == ''){ 
                            $tDisplayKeyProduct = 'show';
                        }else{ 
...
<div class="col-lg-12">
    <div class="row">
        ...
        <?php if($aDataList['rtCode'] == 1){ ?>
            <div class="col-md-6">
                <div class="xWPageListDataPDTReq btn-toolbar pull-right"> 
...
<script>
    ...
    var oCrdBrwCardType = {
		...
		Join		: { 
            ...
            On		: [
                        ...
                        " LEFT JOIN TACTPrDT ON TACTPrDT.FTPdtCode = TCNMPdt.FTPdtCode AND TACTPrDT.FTXrhDocNo = "+tDocumentID+" " +
		...
		Where 		: {
            Condition : [
                ...
                " AND TACTPrDT.FTPdtCode IS NULL "
    ...
    function InsertPDTorBarcode(){
        var tNamerouteInsertBarcode = '<?php echo $tROUTE_omnPurReqCNNew_insertBarcode?>';
        var tFormatCode = $('#ospDocumentnoValue').text();
